...
export class AuthService {
  ...
  async logout(req: Request, res: Response): Promise<{ message: string }> {
    // Dual extraction: Bearer + both plain and __Host- cookies (via cookieExtractor)
    const accessToken =
      ExtractJwt.fromAuthHeaderAsBearerToken()(req) ??
      cookieExtractor('access_token')(req);
    const refreshToken = cookieExtractor('refresh_token')(req);

    // Blacklist access token (covers exp TTL via RedisKeys.auth.blacklist)
    if (accessToken) {
      try {
        await this.tokensService.blacklistToken(accessToken);
      } catch {
        // blacklistToken already swallows verify errors — log already inside service
      }
    }

    // Revoke session via refresh token (even if access expired)
    if (refreshToken) {
      try {
        ...
        // Invalid/expired refresh — still clear cookies below
      }
    }
