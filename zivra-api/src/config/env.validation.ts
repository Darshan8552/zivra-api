...
export const envValidationSchema = Joi.object({
  ...
  // CORS — comma-separated origins, validated as string then split in main.ts
  CORS_ORIGIN: Joi.string()
    ...
    .custom((value, helpers) => {
      ...
      for (const origin of origins) {
        try {
          // Allow localhost http for dev; require https in production is enforced by helmet/hsts later
          // Use URL parsing for stricter validation than regex
    ...
    .messages({
      'any.required': 'CORS_ORIGIN is required (comma-separated origins, e.g. http://localhost:3000)',
      'any.invalid': 'CORS_ORIGIN must be comma-separated valid URLs',
    }),

  // JWT — ensure ≥32 chars to prevent brute-force
  ...
  JWT_RESET_SECRET: Joi.string().min(32).required().messages({
    'any.required': 'JWT_RESET_SECRET is required (min 32 chars) — used for forgot/reset password',
  ...
  // API versioning — URI versioning with defaultVersion
  API_VERSION: Joi.string()
    .pattern(/^\d+$/)
    .default('1')
    .messages({
      'string.pattern.base': 'API_VERSION must be a numeric string like "1" or "2"',
    }),
});
