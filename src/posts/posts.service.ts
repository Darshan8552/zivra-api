...
export class PostsService {
  ...
  private async invalidateFeedCache(authorId: string) {
    try {
      ...
      for (let i = 0; i < userIds.length; i += 500) {
        ...
        await Promise.all(
          chunk.flatMap((uid) => [
            this.redis.scanDel(RedisKeys.cache.feedFollowingPattern(uid)),
            this.redis.scanDel(RedisKeys.cache.feedDiscoveryPattern(uid)),
          ]),
        );
      }
    } catch {}
  ...
  async findOne(postId: string, userId?: string) {
    ...
    if (
      (post.user as unknown as { isPrivate: boolean }).isPrivate &&
  ...
  async searchPosts(dto: SearchPostsDto, viewerId: string) {
    ...
    if (!term) {
      ...
      if (viewerId && items.length > 0) {
        ...
        return {
          items: enriched,
          nextCursor: hasMore ? enriched[enriched.length - 1].id : null,
        };
      }
      return {
    ...
    for (const p of fetched as Array<{
      userId: string;
      user: { isPrivate: boolean };
    }>) {
      authorPrivateMap.set(p.userId, p.user.isPrivate);
      if (p.user.isPrivate && p.userId !== viewerId)
        privateAuthorIds.push(p.userId);
    }
