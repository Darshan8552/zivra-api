...
- [ ] **C-08 Health** — `TerminusModule` `GET /api/health` DB+Redis — `zivra-api/src/app.module.ts:20` | **Verify:** `curl /api/health` 200
...
- [ ] **C-10 Web error boundaries** — `createTanStackRouter({defaultErrorComponent})` + per `_main/feed` — `zivra-web/src/router.tsx:14`, `src/routes/__root.tsx:21` | **Verify:** `throw new Error` shows boundary not white-screen
- [ ] **C-11 Devtools guard** — `import.meta.env.DEV && devtools()` — `zivra-web/vite.config.ts:14`, `src/routes/__root.tsx:54` | **Verify:** `dist` no `TanStackDevtools` strings
...
- [ ] **C-14 Logout revoke** — `backendRequest('/auth/logout')` before local clear + `Session revokedAt` — `zivra-web/src/lib/auth/auth.function.ts:93`, `zivra-api/src/auth/auth.service.ts:372`; fix gateway `bl:`→`auth:bl:` — `...gateway.ts:49` | **Verify:** after logout old `access_token` 401
