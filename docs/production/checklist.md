...
- [x] **C-04 Cookies __Host-** — **DONE 2026-08-29** `zivra-web/src/lib/config/config.ts:7` `Production→production` (was never true) + `zivra-web/src/lib/config/session.server.ts:41` dual `__Host-` delete + dual read `getValidAccessToken` + dual `Secure true/false` clear (stale plain vs __Host) + `zivra-api/src/common/utils/cookies.util.ts:10` `__Host-` prefix + `Path=/` + dual `Secure` clear (migration) | **Verify:** `Set-Cookie: __Host-access_token; Secure; Path=/; SameSite=Lax`
...
- [x] **C-12 Feed validator Zod** — **DONE 2026-08-29** `zivra-web/src/lib/feed/feed.validator.ts:1` `feedQuerySchema` `z.string().uuid().optional()` + `z.coerce.number().int().min(1).max(50).default(12)` + `zivra-web/src/lib/feed/feed.function.ts:13` `.validator(feedQuerySchema)` (was `data=>data` allows `limit:999999`) | **Verify:** `limit:999999` rejected client
- [x] **C-13 401 handling** — **DONE 2026-08-29** `zivra-web/src/lib/feed/feed.function.ts:6` `requireAccessToken` → `redirect /signin` + `try {backendRequest} catch 401 → clearAuthCookies + redirect` + `zivra-web/src/lib/feed/feed.hooks.ts:7` `staleTime 30s, gcTime 5m, retry:(count,err)=>!isAuthError && count<2, refetchOnWindowFocus:false, placeholderData` + `zivra-web/src/lib/config/session.server.ts:41` dual `Secure` clear | **Verify:** expired session redirects not loops
...
- [x] **C-15 TLS + TTL** — **DONE 2026-08-29** `zivra-api/src/redis/redis.service.ts:17` `rediss:// ? {} : undefined` (was inverted) + `zivra-api/src/services/tokens.service.ts:51` `expiresIn: config.get('JWT_ACCESS_EXPIRES_IN') ?? '15m' / '7d'` (was hardcode `15m`/`7d` vs `.env 30d`), `zivra-api/src/common/utils/cookies.util.ts:10` `parseExpiresInToMs(process.env.JWT_*_EXPIRES_IN)` env-driven `maxAge` + `zivra-api/.env:10` `JWT_REFRESH_EXPIRES_IN 30d→7d` align `7d` (code+cookie+web `REFRESH_TOKEN_MAX_AGE 7d`) | **Verify:** `rediss://` TLS + `exp` == `maxAge`
